...
namespace risa {
   ...
   auto OfflineLoader::readInput() -> void {
      Timer tmr1, tmr2;
      std::vector<std::vector<unsigned short>> fileContents(numberOfDetectorModules_);
...
#pragma omp parallel for default(shared) num_threads(numberOfDetectorModules_/3)
      for (auto i = 1; i <= numberOfDetectorModules_; i++) {
         ...
         std::ifstream input(
               path_ + fileName_ + std::to_string(i) + fileEnding_, std::ios::in | std::ios::binary);
         if (!input) {
      ...
      tmr1.stop();
      double totalFileSize = numberOfProjections_*numberOfDetectors_*numberOfPlanes_*numberOfFrames_*sizeof(unsigned short)/1024.0/1024.0;
